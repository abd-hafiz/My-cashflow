      ...
      <aside className="app-sidebar">
        ...
        <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '0 4px 12px' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg,#00D4AA,#00B4D8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '800', color: '#000', flexShrink: 0 }}>
              {userInitial}
            </div>
      ...
      {showDrawer && (
        <>
          ...
          <div className="drawer">
            ...
            {NAV_ITEMS.map(item => (
              <a key={item.path} href={item.path} onClick={() => setShowDrawer(false)} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '13px 12px', borderRadius: '10px', fontSize: '15px', color: pathname === item.path ? '#00D4AA' : '#fff', background: pathname === item.path ? 'rgba(0,212,170,0.1)' : 'transparent', textDecoration: 'none', marginBottom: '4px' }}>
